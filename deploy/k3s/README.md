...
- `20-control-plane.yaml` — `Deployment` (image `ghcr.io/rhanka/sentropic-remote-control-plane:v0.4.0`, liveness/readiness on `/healthz`) + `ClusterIP` Service `sentropic-remote-control-plane` on port 8080. The session-agent images are pulled by the control-plane on demand when a session is created.
