...
- `20-control-plane.yaml` — `Deployment` (image `ghcr.io/sentropic/remote-control-plane:0.1.0`, liveness/readiness on `/healthz`) + `ClusterIP` Service `sentropic-remote-control-plane` on port 8080. The session-agent images are pulled by the control-plane on demand when a session is created.
